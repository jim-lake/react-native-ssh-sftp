...
describe('SSH SFTP Authentication Methods Tests', () => {
  ...
  it('should test encrypted RSA key authentication', async () => {
    ...
    // Check that authentication succeeded
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: Encrypted RSA Key Connected!',
    );
  });
  ...
  it('should test SFTP functionality', async () => {
    ...
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: SFTP Connected! Found 1 files',
    );
  });
  ...
  it('should test RSA 2048 sign callback authentication', async () => {
    ...
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: RSA 2048 Sign Callback Connected!',
    );
  });

  it('should test RSA 2048 SHA256 sign callback authentication', async () => {
    await element(by.id('rsa2048-sha256-sign-callback-button')).tap();
    await new Promise(resolve => setTimeout(resolve, 6800));
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: RSA 2048 SHA256 Sign Callback Connected!',
    );
  });
  ...
  it('should test ECDSA P-256 sign callback authentication', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 6800));
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: ECDSA P-256 Sign Callback Connected!',
    );
  });
  ...
  it('should test Ed25519 sign callback authentication', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 6800));
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: Ed25519 Sign Callback Connected!',
    );
  });
  ...
  it('should test basic SSH functionality', async () => {
    ...
    await detoxExpected(element(by.id('status'))).toHaveText(
      'Status: Native call successful (connection failed as expected)',
    );
  });
});
